...
def get_scanner(Filters):
    ...
    try:
        ...
        for k, v in Filters.items():
            ...
            if value in ExpressionAttributeValues:
                value += str(round(random.random()*10000))
            
            ExpressionAttributeNames[variable] = v['key']
...
def scan_readings(table, filters):
    response = dict()
    response['status'] = True
    try:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        body = None
        if method == "GET":
            resp = get_readings(table=table)
            if resp['status'] == False:
                raise Exception(resp['data'])
            body = resp['data']
        elif method == "POST":
            data = json.loads(event['body'])
            resp = scan_readings(table=table, filters=data)
            if resp['status'] == False:
                raise Exception(resp['data'])
            body = resp['data']
